...
const dateOrderRefinements = <T extends z.ZodTypeAny>(schema: T) =>
  schema
    .refine(
      ...
      { message: "Einreichungs-Deadline muss vor der Bewertungs-Deadline liegen", path: ["submissionDeadline"] }
    )
    .refine(
      (data: z.infer<typeof baseRoundSchema>) => data.ratingDeadline < data.fulfillmentDate,
      { message: "Bewertungs-Deadline muss vor dem Stichtag liegen", path: ["ratingDeadline"] }
    );
